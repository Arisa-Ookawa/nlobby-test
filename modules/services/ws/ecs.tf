...
resource "aws_ecs_task_definition" "this" {
  family = "${var.common.project}-${var.common.environment}-ws-task-${var.sfx}"
  ...
  execution_role_arn = aws_iam_role.ecs_ws_task_role.arn
  task_role_arn      = aws_iam_role.ecs_ws_task_role.arn
  ...
  tags = {
    Name        = "${var.common.project}-${var.common.environment}-ws-task-${var.sfx}"
    Environment = var.common.environment
    Createdby   = "Terraform"
  }
}
...
resource "aws_ecs_service" "this" {
  name = "${var.common.project}-${var.common.environment}-ws-service-${var.sfx}"
  ...
  load_balancer {
    ...
    container_name   = "${var.common.project}-${var.common.environment}-ws-task-${var.sfx}"
    container_port   = 3003
  }
  ...
  network_configuration {
    ...
    security_groups  = ["${aws_security_group.ecs_ws.id}"]
  }
